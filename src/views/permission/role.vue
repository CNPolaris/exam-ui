<template>
  <div class="app-container">
    <div class="filter-container">
      <el-form :model="param" :inline="true">
        ...
        <el-form-item label="编码">
          <el-input v-model="param.code" placeholder="code" style="width: 100px;" class="filter-item" @keyup.enter.native="handleFilter" />
        </el-form-item>
    ...
    >
      ...
      <el-table-column align="center" width="140" label="角色名称">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>

      <el-table-column width="319" align="center" label="描述">
        <template slot-scope="{row}">
          <span>{{ row.description }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" width="100" label="编码">
        <template slot-scope="{row}">
          <span>{{ row.code }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" width="100" label="角色数量">
        <template slot-scope="{row}">
          <span>{{ row.userCount }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" width="190" label="创建时间">
        <template slot-scope="{row}">
          <span>{{ row.createTime| formatDateTime }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" width="190" label="更新时间">
        <template slot-scope="{row}">
          <span>{{ row.updateTime| formatDateTime }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" width="100" label="有效状态">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="300" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <el-button size="mini" type="primary" @click="handleSelectPermission(row,$index)">
            分配权限
          </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="角色编码" prop="url" label-width="140">
...
<script>
import { getRoleList, deleteRole, createRole, updateRole, updateRoleStatus } from '@/api/role'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getRoleList(this.listQuery, this.searchKeyWord).then(response => {
        const { data } = response
        this.list = data.data
    ...
    handleFilter() {
      ...
      this.searchKeyWord.code = this.param.code
    ...
    handleSelectPermission(row, index) {
      this.$router.push({ path: '/permission/allocate', query: { roleId: row.id }})
    },

...
import { monitorServer } from '@/api/monitor'
export default {
  name: 'ServerMonitor',
  components: {
    'v-chart': ECharts
  ...
  methods: {
    init() {
      monitorServer().then(response => {
        ...
        this.cpuInfo.xAxis.data.push(response.data.time)
        this.memoryInfo.xAxis.data.push(response.data.time)
        this.cpuInfo.series[0].data.push(parseFloat(response.data.cpu.used))
        this.memoryInfo.series[0].data.push(parseFloat(response.data.memory.usageRate))
      })
    }
  }
}
